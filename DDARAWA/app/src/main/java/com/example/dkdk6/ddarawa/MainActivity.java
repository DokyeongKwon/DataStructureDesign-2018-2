...
public class MainActivity extends AppCompatActivity {
                ...
                if(!address.getText().toString().equals(null)){
                    ...
                    if (list != null) {
                        if (list.size() == 0) {
                            ...
                            intent.putExtra("city", select+":"+select_finedust+":"+myX+":"+myY);
    ...
    public void selectCityAlgorithm(String r_ad){
       ...
       // Log.d("ADDRESS", myAddress);
        ...
        ArrayList<CityGraph> cityNodes = new ArrayList<CityGraph>();
        for(int b=0; b<RcityList.size(); b++){
            CityGraph tempNode = new CityGraph(myLocation, RcityList.get(b));
    ...
    public void computeMaxDistance(ArrayList<CityGraph> r_node){
    ...
    public void computeMaxPopulation(ArrayList<CityGraph> r_node){
    ...
    public void computeWeight(ArrayList<CityGraph> r_node){
        ...
        for(int i=0; i<r_node.size(); i++){
                r_node.get(i).weight = ((r_node.get(i).distance/r_node.get(i).maxDistance)*weightD)+((r_node.get(i).population/r_node.get(i).maxPopulation)*weightP);
           // r_node.get(i).weight = (r_node.get(i).distance*weightD)+(r_node.get(i).population*weightP);
      ...
      /*다익스트라 알고리즘으로 최단경로 찾기.*/
        ...
        for(int i=0; i<r_node.size(); i++){
            if(minWeight==r_node.get(i).weight){
                ...
                myX = r_node.get(i).computCity.x_axis;
                myY =r_node.get(i).computCity.y_axis;
            }
        }
